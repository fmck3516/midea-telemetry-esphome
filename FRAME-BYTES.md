...
| **decoded** | The firmware decodes it into a sensor. The formula is in the [README](README.md#supported-sensors). |
...
|---|---|---|---|---|---|
| 2 | T1 | Indoor ambient temperature | NTC β-model. `0x00` and `0xFF` are fault codes. | decoded | `indoor_ambient_temperature` |
| 3 | T2 | Indoor coil temperature | NTC β-model | decoded | `indoor_coil_temperature` |
| 4 | T3 | Outdoor coil temperature | NTC β-model | decoded | `outdoor_coil_temperature` |
| 5 | T4 | Outdoor ambient temperature | NTC β-model | decoded | `outdoor_ambient_temperature` |
| 6 | TP | Compressor discharge temperature | Steinhart–Hart. `0x00`, `0xFE` and `0xFF` are fault codes. | decoded | `discharge_temperature` |
...
| Byte | Code | Meaning | Encoding | Status | Source |
|---|---|---|---|---|---|
| 2 | dL | Compressor current draw | `0.117 · b + 0.92` A, but only while the compressor runs. Otherwise the byte sits at a per-unit floor. | decoded | `current_draw`, [#16](https://github.com/fmck3516/midea-telemetry-esphome/issues/16) |
...
| 7 | TT | Indoor set-point | `b < 50 ? b : (b − 50) / 2` °C. Two OEM encodings are in use. | decoded | `indoor_setpoint` |
...
| 6 | — | — | — | unknown | |
| 7 | — | — | — | unknown | |
| 8 | — | Operating mode | Raw code: 0 OFF, 1 COOL, 2 HEAT, 3 ONLY FAN, 4 DRY, 5 RESERVED, 6 FORCE COOL, 7 DEFROST | decoded | `operating_mode` |
